...
from floranet.floranet.lora.crypto import aesEncrypt, aesDecrypt
from floranet.floranet.util import intPackBytes, intUnpackBytes
from floranet.floranet.error import DecodeError
...
class JoinRequestMessage(MACMessage):
    ...
    def checkMIC(self, appkey):
        ...
        """
        data = bytes.fromhex(self.mhdr) + struct.pack('<QQH', int(self.appeui,16), int(self.deveui,16), int(self.devnonce,16))
        aesdata = aesEncrypt(intPackBytes(appkey, 16), data, mode='CMAC')
        mic = struct.unpack('<L', aesdata[:4])[0]
        mic = '{:x}'.format(mic)
        mic = "".join(reversed([mic[i:i+2] for i in range(0, len(mic), 2)]))
        return mic.lower() == self.mic.lower()
    
    def derive_skey(self, key_prefix, JAMObject):
        """
        Args:
            pre (int): 0x01 for NwkSKey, 0x02 for AppSKey
        ...
        data = struct.pack('B', key_prefix) + \
               intPackBytes(JAMObject.appnonce, 3, endian='little') + \
               intPackBytes(JAMObject.netid, 3, endian='little') + \
               struct.pack('<H', int(self.devnonce, 16)) + intPackBytes(0, 7)
        aesdata = aesEncrypt(intPackBytes(JAMObject.appkey, 16), data)
...
class JoinAcceptMessage(MACMessage):
    ...
    def encode(self):
        ...
        print("Before AES: ", (header+msg).hex())
